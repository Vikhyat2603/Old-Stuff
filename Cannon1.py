...
print("Commands: [Break] ; [Rotate] ; [Shoot] ; Input is NOT case-sensitive so you can type \"sHOoT\" instead os \"shoot\" ; For rotation mention the angle in degrees: the program rounds it off ; Shooting power is from 0 to 100 ; Sample input: \"rotate 50\" or \"shoot 100\"")
screen = Screen()
...
while 1:
          ...
          if(inp=="break"):
                    bye()
                    break
          if((str_ops.substring(inp,0,4))=="shoot"):
                    inp=round(float((str_ops.substring(inp,6,len(inp)-1)))/5)
                    shoot(inp)
                    continue
          elif((str_ops.substring(inp,0,5))=="rotate"):
                    inp=round(float((str_ops.substring(inp,7,len(inp)-1))))
                    rot+=inp
                    rot%=360
                    rotate(inp)
                    render("cannon")
